...
/* ============================================================
   ...
   ============================================================ */
const TROOPS=[
 {id:'militia',  name:'Militia',     cost:15,  hp:52,  dmg:5,  rate:1.0, speed:70,  melee:true,  unlock:1,  snd:'melee_hit1', desc:'Cheap fodder. Holds the line.'},
 {id:'archer',   name:'Archer',      cost:25,  hp:32,  dmg:7,  rate:0.9, speed:70,  melee:false, range:145, unlock:1,  snd:'arrow', desc:'Shoots from a distance.'},
 {id:'sword',    name:'Swordsman',   cost:35,  hp:95,  dmg:9,  rate:0.85,speed:66,  melee:true,  unlock:2,  snd:'melee_hit2', desc:'Solid frontline fighter.'},
 {id:'spear',    name:'Spearman',    cost:45,  hp:78,  dmg:12, rate:1.0, speed:66,  melee:true,  unlock:4,  bonusFast:1.5, snd:'melee_hit3', desc:'+50% damage vs fast enemies.'},
 {id:'xbow',     name:'Crossbowman', cost:55,  hp:42,  dmg:17, rate:1.3, speed:62,  melee:false, range:175, pierceArmor:0.5, unlock:6, snd:'crossbow', desc:'Bolts punch through 50% armor.'},
 {id:'berserker',name:'Berserker',   cost:70,  hp:75,  dmg:26, rate:0.9, speed:78,  melee:true,  unlock:8,  snd:'melee_hit2', desc:'Attacks faster as he bleeds.'},
 {id:'knight',   name:'Knight',      cost:90,  hp:270, dmg:10, rate:1.0, speed:56,  melee:true,  armor:0.3, unlock:10, snd:'melee_hit1', desc:'Heavy armor. Great blocker.'},
 {id:'mage',     name:'Battle Mage', cost:110, hp:58,  dmg:21, rate:1.6, speed:60,  melee:false, range:155, splash:45, unlock:12, snd:'mage_bolt', desc:'Explosive magic bolts (splash).'},
 {id:'cleric',   name:'Cleric',      cost:100, hp:75,  dmg:0,  rate:0,   speed:62,  melee:false, heal:14, healRange:100, unlock:14, snd:'holy_heal', desc:'Heals your troops and heroes.'},
...
/* ============================================================
   ...
   ============================================================ */
const HEROES=[
 {id:'aldric', name:'Sir Aldric', title:'Knight-Commander', unlockWave:0, cost:0,
  hp:420, dmg:34, rate:0.7, speed:95, melee:true, cleave:48, col:'#7cc4ff', cape:'#c23a3a', snd:'hero_aldric_atk',
  skill:{id:'slam', name:'Valor Slam', unlockLvl:3, cd:9, snd:'skill_slam',
    desc:'Shockwave slam: heavy AoE damage + brief stun.'},
  passive:{lvl:8, name:'War Banner', desc:'Troops near Aldric deal +15% damage.'}},
 {id:'lyra', name:'Lyra Swiftwind', title:'Warden of the Glade', unlockWave:20, cost:600,
  hp:300, dmg:26, rate:0.45, speed:105, melee:false, range:180, col:'#7ede8a', cape:'#3a7a44', snd:'hero_lyra_atk',
  skill:{id:'arrowstorm', name:'Arrow Storm', unlockLvl:3, cd:12, snd:'skill_arrowstorm',
    desc:'Rains a volley of 12 arrows on the thickest pack.'},
  passive:{lvl:8, name:'Eagle Eye', desc:'+20% range, 15% chance to crit for double.'}},
 {id:'magnus', name:'Magnus Emberforge', title:'Archmage of Cinders', unlockWave:30, cost:1200,
  hp:280, dmg:38, rate:1.4, speed:85, melee:false, range:165, splash:50, dtype:'magic', col:'#ff9a5e', cape:'#8a2a1a', snd:'hero_magnus_atk',
  skill:{id:'meteor', name:'Meteor', unlockLvl:3, cd:14, snd:'skill_meteor',
    desc:'Calls a meteor onto the largest cluster: massive burn damage.'},
  passive:{lvl:8, name:'Cinder Touch', desc:'His attacks set enemies ablaze.'}},
 {id:'celeste', name:'Sister Celeste', title:'Voice of the Dawn', unlockWave:40, cost:2000,
  hp:340, dmg:20, rate:1.0, speed:88, melee:false, range:150, dtype:'magic', col:'#ffe8a8', cape:'#e8c93a', snd:'hero_celeste_atk',
  skill:{id:'sanctuary', name:'Sanctuary', unlockLvl:3, cd:16, snd:'skill_sanctuary',
    desc:'Heals your whole army 45% and shields it for 4s.'},
  passive:{lvl:8, name:'Dawn Aura', desc:'Allies near Celeste regenerate.'}},
 {id:'bjorn', name:'Bjorn Ironhide', title:'Jarl of the North', unlockWave:50, cost:3200,
  hp:780, dmg:42, rate:0.95, speed:80, melee:true, col:'#e0b060', cape:'#6a4a8a', snd:'hero_bjorn_atk',
  skill:{id:'warcry', name:'War Cry', unlockLvl:3, cd:13, snd:'skill_warcry',
    desc:'Terrifying roar: stuns nearby enemies, mends his wounds.'},
  passive:{lvl:8, name:'Iron Hide', desc:'Takes 30% less damage.'}},
 {id:'nyx', name:'Nyx Shadowblade', title:'Whisper of Midnight', unlockWave:60, cost:5000,
...
function vaultData(){
  ...
  try{VAULT=JSON.parse(localStorage.getItem('rs2_vault'))||null;}catch(err){VAULT=null;}
...
function vaultSave(){try{localStorage.setItem('rs2_vault',JSON.stringify(vaultData()));}catch(err){}}
...
/* ============================================================
   ...
   ============================================================ */
const RELICS=[
 {id:'steel',      name:'Sharpened Steel',  max:5, base:300, mult:1.9,  eff:'troopDmg', per:0.10, desc:'+10% troop damage per tier.'},
 {id:'engineering',name:'Siege Engineering',max:5, base:350, mult:1.9,  eff:'towerDmg', per:0.08, desc:'+8% tower damage per tier.'},
 {id:'banners',    name:'War Banners',      max:5, base:400, mult:1.85, eff:'armyCap',  per:2,    desc:'+2 army capacity per tier.'},
 {id:'treasury',   name:'Royal Treasury',   max:5, base:500, mult:1.95, eff:'gold',     per:0.10, desc:'+10% gold from all sources per tier.'},
 {id:'walls',      name:'Bastion Walls',    max:5, base:450, mult:1.85, eff:'lives',    per:5,    desc:'+5 max castle lives per tier (and repairs 5).'},
 {id:'grimoire',   name:"Hero's Grimoire",  max:5, base:400, mult:1.9,  eff:'heroPower',per:0.10, desc:'+10% hero damage & health per tier.'},
 {id:'drums',      name:'Drums of War',     max:3, base:600, mult:2.0,  eff:'summon',   per:0.25, desc:'Army resummons 25% faster per tier.'},
];
